...
def preprocess(img, h, w):
    ...
    img = tf.cast(tf.concat(axis=2, values=[img_b, img_g, img_r]), dtype=tf.float32)  #tf.concat在axis维度拼接张量，其他维度不变；tf.cast将x的数据格式转换成dtype
    # Extract mean.
    img -= IMG_MEAN

    pad_img = tf.image.pad_to_bounding_box(img, 0, 0, h, w) #用0指定的heigh和width来填充image
    pad_img = tf.expand_dims(pad_img, dim=0) #将维度增加一维
