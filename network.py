...
def layer(op):
    """Decorator for composable network layers."""  #Decorator装饰 composable组成

    """
    *args和**kwargs用于函数定义，将不定量的参数传送给函数
    其中，*args用于发送一个非键值对的可变数量的参数列表给一个函数，argument表示位置参数
    **kwargs，即keyword argument，表示关键字参数，将一个可变的关键字参数的字典传给函数实参
    """
    def layer_decorated(self, *args, **kwargs):
        # Automatically set a name if not provided.
        name = kwargs.setdefault('name', self.get_unique_name(op.__name__))   #.setdefault是dict的函数
...
class Network(object):
    ...
    '''Define: inputs ; terminals; layers; trainable; use_dropout'''
    ...
    def setup(self, is_training):
        ...
        raise NotImplementedError('Must be implemented by the subclass.')  #subclass 子集
    ...
    def load(self, data_path, session, ignore_missing=False):
        ...
        '''
        data_dict = np.load(data_path, encoding='latin1').item()  #latin1是一种类似ASCII码、UTF8的编码格式

        for op_name in data_dict:
            with tf.variable_scope(op_name, reuse=True): #用于定义创建变量(层)的操作的上下文管理器.
